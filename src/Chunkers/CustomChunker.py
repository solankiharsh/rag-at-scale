...
class CustomChunker(Chunker):
    ...
    def chunk(self, documents:List[RagDocument]) -> Generator[List[RagDocument], None, None]:
        ...
        for doc in documents:
            ...
            for i in range(len(chunks)):
                documents_to_embed.append(RagDocument(id=doc.id + "_" + str(i), content=chunks[i].page_content, metadata=doc.metadata))
    ...
    def config_validation(self) -> bool:
        try:
            ...
            raise ImportError("You must run " "`pip install neumai-tools")
        try:
            chunks = semantic_chunking(documents=[RagDocument(id="test", content="test", metadata={})], chunking_code_exec=self.code)
