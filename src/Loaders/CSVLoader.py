...
class CSVLoader(Loader):
    ...
    A utility class for loading and processing CSV files. This loader is designed to handle various
    CSV formats and configurations, making it flexible for different data loading requirements.

    Attributes:
    -----------
    id_key : Optional[str]
        An optional ID key that can be used to identify unique records within the CSV file.
        If provided, it specifies the column name that contains unique identifiers.

    source_column : Optional[str]
        An optional source column name from which to load data.
        If specified, only this column will be used for further processing.

    encoding : Optional[str]
        An optional encoding type for reading the CSV file. This should be a valid encoding type
        understood by Python's CSV parser.

    csv_args : Optional[Dict]
        Optional additional arguments that can be passed to the CSV reader.
        This can include settings like delimiter, quotechar, etc.

    selector : Optional[Selector]
        An optional Selector object to define criteria for selecting, embedding, or modifying
        metadata in the data. Default is a Selector with empty 'to_embed' and 'to_metadata' lists.
    """
