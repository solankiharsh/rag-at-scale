...
class Config:
    ...
    ham_embeddings_endpoint: str = "https://stgapi-internal.target.com/hosted_ai_models/v1/text/embed?model=jina-v2-base"
    ...
    db_entitlement_protection: bool = (os.getenv("DB_ENTITLEMENT_PROTECTION", "False").lower() == "true")
    ...
    supported_chunking_strategies: str = os.getenv("SUPPORTED_CHUNKING_STRATEGIES", "semantic,markdown,recursive")
    ...
    upload_formats_supported: str = os.getenv("UPLOAD_FORMATS_SUPPORTED", "txt,pdf,docx,pdf2,pptx,adoc,md")
    ...
    max_rectangle_over_image_intersection: float = float(os.getenv("MAX_RECTANGLE_OVER_IMAGE_INTERSECTION", "0.8"))
    max_rectangle_over_table_intersection: float = float(os.getenv("MAX_RECTANGLE_OVER_TABLE_INTERSECTION", "0.4"))
    max_table_over_rectangle_intersection: float = float(os.getenv("MAX_TABLE_OVER_RECTANGLE_INTERSECTION", "0.5"))
    max_table_over_image_intersection: float = float(os.getenv("MAX_TABLE_OVER_IMAGE_INTERSECTION", "0.5"))
    max_text_over_rectangle_intersection: float = float(os.getenv("MAX_TEXT_OVER_RECTANGLE_INTERSECTION", "0.8"))
    max_text_over_image_intersection: float = float(os.getenv("MAX_TEXT_OVER_IMAGE_INTERSECTION", "0.8"))
    max_text_over_table_intersection: float = float(os.getenv("MAX_TEXT_OVER_TABLE_INTERSECTION", "0.2"))
    max_image_over_rectangle_intersection: float = float(os.getenv("MAX_IMAGE_OVER_RECTANGLE_INTERSECTION", "0.8"))
    max_image_over_table_intersection: float = float(os.getenv("MAX_IMAGE_OVER_TABLE_INTERSECTION", "0.5"))
    ...
    redis_cache_workflow_retries: bool = (os.getenv("REDIS_CACHE_WORKFLOW_RETRIES", "False").lower() == "true")
    ...
    redis_memory_limit: int = int(os.getenv("REDIS_MEMORY_LIMIT", str(512 * 1024 * 1024)))  # Need to stringify for getenv then convert to int
    ...
    uvicorn_worker_timeout_keep_alive: int = int(os.getenv("UVICORN_WORKER_TIMEOUT_KEEP_ALIVE", "5"))
    # gunicorn additional configs
    uvicorn_worker_preload_app: bool = (os.getenv("UVICORN_WORKER_PRELOAD_APP", "True").lower() == "true") # Note default changed to True to match original Settings
    ...
    uvicorn_worker_max_requests_jitter: int = int(os.getenv("UVICORN_WORKER_MAX_REQUESTS_JITTER", "0"))

    # Embeddings configurations
    # Embedding models with configurable dimensions from .env
    text_embedding_3_small_dimensions: list[int] = [int(x) for x in os.getenv("TEXT_EMBEDDING_3_SMALL_DIMENSIONS", "256,512,1024,1536").split(',')]
    text_embedding_3_large_dimensions: list[int] = [int(x) for x in os.getenv("TEXT_EMBEDDING_3_LARGE_DIMENSIONS", "256,1024,3072").split(',')]
    text_embedding_ada_002_dimensions: list[int] = [int(x) for x in os.getenv("TEXT_EMBEDDING_ADA_002_DIMENSIONS", "1536").split(',')]
    ...
    quota_endpoint: str = os.getenv("QUOTA_ENDPOINT", "https://ttassimilator.dev.target.com/v1/quota_service")
    quota_check: bool = (os.getenv("QUOTA_CHECK", "False").lower() == "true")

    # Usage History
    kafka_ca_location: str = os.getenv("KAFKA_CA_LOCATION", "/usr/local/share/ca-certificates/tgt-ca-bundle.crt")
    kafka_certificate_location: str = os.getenv("KAFKA_CERTIFICATE_LOCATION", "/tap/certificates/cert.pem")
    ...
    kafka_usage_history_topic: str = os.getenv("KAFKA_USAGE_HISTORY_TOPIC", "genai-platform-dev-usage-history")
